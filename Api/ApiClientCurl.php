...
class ApiClientCurl implements ApiClientInterface
{
    public const CURL_ERROR_CODES = [
    ...
    public function send(): ApiResponseInterface
    {
        ...
        curl_setopt($this->ch, CURLOPT_URL, $this->request->getUri());
        curl_setopt($this->ch, CURLOPT_CUSTOMREQUEST, $method);

        if (in_array($method, ['GET']) === false) {
            curl_setopt($this->ch, CURLOPT_POST, true);
            curl_setopt($this->ch, CURLOPT_POSTFIELDS, $this->request->getBody());
        }
        ...
        $response = new ApiResponse($this->request);

        if ($res === false) {
            $error_code = curl_errno($this->ch);
            $error_message = curl_error($this->ch);

            if (array_key_exists($error_code, self::CURL_ERROR_CODES)) {
                $error_code = self::CURL_ERROR_CODES[$error_code];
            } else {
                $error_code = 'CURLE_NOT_DEFINED';
            }

            $response->setError($error_code, $error_message);
        } else {
            $response->setStatusCode(curl_getinfo($this->ch, CURLINFO_RESPONSE_CODE));
